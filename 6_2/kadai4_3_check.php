...
else
{
	...
	旧郵便番号 ： {$zip_code_old}<br><br>
	郵便番号 ： {$zip_code}<br><br>
	都道府県名(半角カタカナ) ： {$prefecture_kana}<br><br>
	市区町村名(半角カタカナ) ： {$city_kana}<br><br>
	町域名(半角カタカナ) ： {$town_kana}<br><br>
	都道府県名(漢字) ： {$prefecture}<br><br>
	市区町村名(漢字) ： {$city}<br><br>
	町域名(漢字) ： {$town}<br><br>
	一町域で複数の郵便番号か ： {$town_double_zip_code}<br><br>
	小字毎に番地が起番されている町域か ： {$town_multi_address}<br><br>
	丁目を有する町域名か ： {$town_attach_district}<br><br>
	一郵便番号で複数の町域か：{$zip_code_multi_town}<br><br>
	更新確認 ： {$update_check}<br><br>
	更新理由 ： {$update_reason}<br><br>

 <br><br><br>
 <p>上記の内容に編集しますか？</p>
<form action="kadai4_3_updata.php" method="post">
